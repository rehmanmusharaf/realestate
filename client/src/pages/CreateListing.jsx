...
export default function CreateListing() {
  ...
  const handleImageUpload = (event) => {
    ...
    if (file) {
      ...
      setImageSrc(src);
      classifyImage(src);
      setFiles([file]); // Update the files state with the new file
    }
  };
  ...
  const handleImageSubmit = (e) => {
    ...
    // let check= validClasses.includes(prediction.className.toLowerCase())
    console.log("check log",check);
        if (files.length > 0 && files.length + formData.imageUrls.length < 7 &&check) {
      setUploading(true);
    ...
    predictions.slice(0, 1).map((prediction, index) => (
      <p key={index}>
        {validClasses.includes(prediction.className.toLowerCase())
          ...
          :`Error: The image is of a ${prediction.className}, which is not allowed.` }
      </p>
    ))}
</div>
              {/* <div id="predictions">
